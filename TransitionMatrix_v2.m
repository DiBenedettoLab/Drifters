...
%% SPECIFICATIONS ------------------------------------------------------ %%
bcrit=10; %beaching distance. 
makecoastdirmat=false;
makevelmat=false;
maketimemat=true;
makedirfinalmat=true;
cutoffdays=365;
...
[fig,xg,yg,xpt,ypt,hmap]=make_grid_fig('area',dt,1,[coast_lat',coast_lon']);
    ...
    for j=1:length(ds(i).lat)
        ...
        if maketimemat
            if ds(i).coast(end)<=bcrit
                timetemp=mTime(row,col)+ds(i).days_norm(end)-ds(i).days_norm(j);
                if timetemp<=cutoffdays
                    mTime(row,col)=timetemp;
                    mTimePing(row,col)=mTimePing(row,col)+1;
                    time_ping_mat_temp(row,col)=time_ping_mat_temp(row,col)+1;
                end
            end
        end

        %DIRECTION OF FINAL BEACH
        if makedirfinalmat
            if ds(i).coast(end)<=bcrit
                %finding shortest latitude distance
                if sum(ds(i).lon<0)>0
                    lonshift=ds(i).lon;
                    lonshift(ds(i).lon<0)=lonshift(ds(i).lon<0)+360;
                    lon1=lonshift(end)-lonshift(j);
                else
                    lon1=[];
                end
                lon2=ds(i).lon(end)-ds(i).lon(j);
                lons=[lon1,lon2];
                [~,indx]=min(abs(lons));

                cCoastDirLat{row,col}=[[cCoastDirLat{row,col}],ds(i).lat(end)-ds(i).lat(j)];
                cCoastDirLon{row,col}=[[cCoastDirLon{row,col}],lons(indx)];
            end
        end
...
if minimumdrifters>0
    ...
    if maketimemat
        DataMinTime=mTimeUniqueID<minimumdrifters;
        mTimemean(DataMinTime)=NaN;
    end
else
...
%TIME MAT
if maketimemat
    mTimemean=mTime./mTimePing;
end

%DIRECTION OF BEACHING
if makedirfinalmat
    mCoastDirLatMean=zeros(size(xg));
    mCoastDirLonMean=zeros(size(xg));
    for ii=1:numel(cCoastDirLat)
        if ~isempty(cCoastDirLat{ii})
            mCoastDirLatMean(ii)=mean(cCoastDirLat{ii});
            mCoastDirLonMean(ii)=mean(cCoastDirLon{ii});
        end
    end
    mCoastDirMag=sqrt(mCoastDirLatMean.^2+mCoastDirLonMean.^2);
    mCoastDirLat=mCoastDirLatMean./mCoastDirMag;
    mCoastDirLon=mCoastDirLonMean./mCoastDirMag;
    mCoastDirLatNorm=mCoastDirLat.*mProb;
    mCoastDirLonNorm=mCoastDirLon.*mProb;
end
...
switch data2plot
    ...
    case 'time'
        if maketimemat
            plotting_mat=mTimemean;
            plotting_name='Mean time to beach';
            ax=gca; %makes NaN values grey
            colormap(parula);
        end
    case 'coastdirec'
